...
<html lang="en" dir="ltr">
  <head>
    ...
    <title>{SITE_NAME}: Portal Manager</title>
    {STYLESHEETS}
  </head>
  <body>
        ...
        <div class="col-md-6">
          <div class="Box">
            <div class="Title">
              ...
              <div class="float-right">
                <div class="btn-group">
                  <button class="btn btn-primary" data-toggle="modal" data-target="#AddNewPortalBay_Modal">Add a Bay</button>
                  <button class="btn btn-primary" onClick="DownloadBays_Manage()"><i class="fa fa-sync-alt"></i></button>

                </div>
              </div>
            </div>
            <div id="Portal_BayData_Manage">
    ...
    <script type="text/javascript">
      ...
      $(document).ready(function() {
        $.ajax({
          url: "{URL}/core/ajax/vehicles.handler.php?handler=Vehicles.List_PortalBays",
          method: "POST",
          dataType: "text",
          success:function(Response) {
            $('#Portal_BayData_Manage').html(Response);
          }
        })
      });
      function DownloadActivePortalBookings_Manage() {
        $.ajax({
          url: "{URL}/core/ajax/vehicles.handler.php?handler=Vehicles.List_PortalBookings",
          method: "POST",
          dataType: "text",
      ...
      $(document).on('submit', '#AddNewPortalBay_Form', function() {
        event.preventDefault();
        var Data = $(this).serialize();
        $.ajax({
          url: "{URL}/core/ajax/vehicles.handler.php?handler=Vehicles.AddBayToPortal",
          data: Data,
          method: "POST",
          dataType: "json",
