<?php
  ...
  class Ticket {
    ...
    function Printer_Columns($leftCol, $rightCol, $leftWidth, $rightWidth, $space = 4) {
    ...
    function Printer_ParkingTicket($ticket_name, $gross, $net, $company, $reg, $tid, $date, $expiry, $payment_type, $meal_count, $shower_count)
    {
      ...
      $logo = EscposImage::load(URL."assets/img/".$campus."/logo.png", false);
      $address = EscposImage::load(URL."assets/img/".$campus."/address.png", false);
      $shower_img = EscposImage::load(URL."assets/img/".$campus."/shower.jpg", false);
      $meal_img = EscposImage::load(URL."assets/img/".$campus."/meal.jpg", false);
      $date = date("d/m/Y H:i", strtotime($date));
      $expiry = date("d/m/Y H:i", strtotime($expiry));

      //Sorting column data
      $line_one = $this->Printer_Columns("Company: ".$company, "Reg: ".$reg);
      $line_two = $this->Printer_Columns("ID: ".$tid, "Date: ".$date);
      $line_info = $this->Printer_Columns("Reg: ".$reg, "Exp: ".$expiry);
      //Begin print
      //Ticket Code
      try {
        ...
        //End Parking ticket
        $printer -> cut();
        //Begin Meal + Shower LOOPS
        $count_shower = 0;
        $count_meal = 0;
        while ($shower_count > 0 AND $count_shower <= $shower_count) {
          //Shower Ticket
          $count_shower++;
          $printer -> setJustification(Printer::JUSTIFY_CENTER);
        ...
        while ($meal_count > 0 AND $count_meal <= $meal_count) {
          //Shower Ticket
          $count_meal++;
          $printer -> setJustification(Printer::JUSTIFY_CENTER);
