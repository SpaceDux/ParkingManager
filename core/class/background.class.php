  ...
  class Background {
     ...
     function Automation_Exit()
     {
       ...
       foreach ($anpr->fetchAll() as $row) {
         ...
         // echo '<br>'.$plate.' '.$date;
         $parking = $this->mysql->dbc->prepare("SELECT * FROM pm_parking_records WHERE Plate = ? AND Parked_Column = 1 AND Site = ? ORDER BY Arrival DESC");
         ...
         if($date > $timein) {
           ...
           if($parked_expiry >= $expiry) {
             ...
             if($count < 1) {
               $query = $this->mysql->dbc->prepare("UPDATE pm_parking_records SET Departure = ?, Parked_Column = '2' WHERE Uniqueref = ?");
