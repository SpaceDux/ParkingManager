  ...
  class Background {
     ...
     function Blacklist_Check()
     {
       ...
       if($_CONFIG['ANPR']['Type'] == "ETP") {
         ...
         $stmt = $this->rev->dbc->prepare("SELECT * FROM rev_plates WHERE LaneID = 1 AND Status < 1 ORDER BY CaptureTime DESC LIMIT 200");
         $stmt->execute();
         foreach($stmt->fetchAll() as $row) {
           $stmt = $this->mysql->dbc->prepare("SELECT * FROM blacklists WHERE Plate = ?");
           $stmt->bindValue(1, $row['Plate']);
     ...
     function ANPR_PortalCheck()
     {
       ...
       if($_CONFIG['ANPR']['Type'] == "ETP") {
         ...
         $stmt = $this->rev->dbc->prepare("SELECT Plate FROM rev_plates WHERE LaneID = 1 AND Status < 1 ORDER BY CaptureTime DESC");
         $stmt->execute();
         foreach($stmt->fetchAll() as $row) {
           foreach($Bookings['Data'] as $Portal) {
             if($Portal['Plate'] == $row['Plate']) {
